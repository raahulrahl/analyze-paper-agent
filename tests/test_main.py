...
from analyze_paper_agent.main import handler
...
async def test_handler_returns_response():
    ...
    with patch("analyze_paper_agent.main._initialized", True), \
         patch("analyze_paper_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response):
...
async def test_handler_with_multiple_messages():
    ...
    with patch("analyze_paper_agent.main._initialized", True), \
         patch("analyze_paper_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with patch("analyze_paper_agent.main._initialized", False), \
         patch("analyze_paper_agent.main.initialize_all", new_callable=AsyncMock) as mock_init, \
         patch("analyze_paper_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response), \
         patch("analyze_paper_agent.main._init_lock"):
